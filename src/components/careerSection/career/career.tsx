...
export const Career = () => {
  ...
  const handleData = async() => {
    setIsLoading(true)
    try {
    const {data} = await axios.get(`${URL}/api/v1/admin/Career/allCareer`)
...
const handleDeleteArticle = async(id : string) => {
  ...
  setSelectedId(id)
  try {
    const {data} = await axios.delete(`${URL}/api/v1/admin/Career/deleteCareer/${id}`)
...
const displayHeader = () => {
  let headers = ["Image", "Title", "Description", "Operations"]
  return(
      headers.map((item) => {
          return(
