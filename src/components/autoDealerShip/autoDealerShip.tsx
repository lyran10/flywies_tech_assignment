...
import { CreateAutoDealerShip } from './createAutoDealerShip'

export const URL = import.meta.env.VITE_API_URL

export const AutoDealerShip = () => {
...
//   setSelectedId(id)
//   try {
//   const {data} = await axios.delete(`${URL}/api/v1/admin/AutoDealerShip/deleteAutoDealerShip/${id}`)
//   if(data.msg === "deleted"){
//     setMsg({status : "success", content : "Deleted Successfully.", bg : "bg-green-500"}) 
...
// }
   ...
   useEffect(() => {
      ...
      if(!openCreate){
       ...
       if(!openCreate) handleData()
      }
      return () => clearTimeout(timerRef.current)
    },[openCreate])

const displayHeader = () => {
  let headers = ["Title", "Description", "Promoted Placement", "Every Thing Heading", "Operations"]
  return(
      headers.map((item) => {
          return(
          ...
          return (
              <tr key={index} className={`border-b border-slate-300`}>
                 <td className="text-left p-2">
                  ...
                  <td className="text-left p-2">{item.promotedPlacementHeading}</td>
                  <td className="text-left p-2">{item.everyThingHeading}</td>
                  <td className="text-center p-2 flex justify-center items-center gap-2">
                    ...
                    <Button id="" type="button" onClick={() => {}} className='text-[rgba(255,0,0,1)] px-4 py-1 bg-[rgba(255,0,0,0.1)]'>Delete</Button>
                    {/* {selectedId === item._id && deleteLoading ? "Deleting..." : "Delete"} */}
                  </td>
              </tr>
          )
      })
