...
async function handleSubmit (_: any, formData: FormData) {
  ...
  try {
    const { data } = await axios.post(`${URL}/api/v1/admin/Article/createArticle`,formData);
    if(data.status === 200) setMsg({ status: "success", content: "Article created successfully", bg : "bg-green-500" })
